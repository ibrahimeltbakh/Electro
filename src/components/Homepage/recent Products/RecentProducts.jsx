...
const RecentProducts = ({ numberOfProducts, start, name, showSlider = false }) => {
  start = start || 0;
  ...
  const sliderProducts = name === 'Best Seller' ? 
    data.products.slice(0, 6) : 
    data.products.slice(start, start + Math.min(6, numberOfProducts));
    
  const gridProducts = name === 'Best Seller' ? 
    data.products.slice(6, 6 + numberOfProducts - 6) : 
    data.products.slice(start + Math.min(6, numberOfProducts), start + numberOfProducts);
  ...
  return (
    <section className="py-12 overflow-hidden">
      <div className="container mx-auto px-4">
        ...
        {(name === 'Best Seller' || showSlider) && sliderProducts.length > 0 && (
