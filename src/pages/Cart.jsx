...
export default function Cart() {
  ...
  const handleCoupon = (couponCode) => {
    const couponExist = coupons?.coupons?.find(
      (coupon) => coupon.code === couponCode
    );
    if (!couponExist) {
      setCouponStatus("this coupon is not found");
      setCouponCode("");
      return;
    }
    if (couponExist) {
      if (new Date(couponExist.toDate) < new Date()) {
        setCouponStatus("this coupon is expired");
        setCouponCode("");
        return;
      } else {
        setfinalTotal(
          data?.cart?.totalCartPrice -
            (data?.cart?.totalCartPrice * couponExist?.amount) / 100
        );
        setCouponCode("");
        setUserTakeCoupon(true);
  ...
  if (isLoading && !data?.cart?.totalCartPrice) {
    return <Loading />;
  }
        ...
        <motion.div variants={itemVariants} className="mb-8">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <motion.div whileHover={{ x: -5 }} whileTap={{ scale: 0.95 }}>
              <Link
                to="/shop"
                className="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 flex items-center gap-2 transition-colors px-4 py-2 rounded-lg hover:bg-blue-50 dark:hover:bg-blue-900/20">
                <FaArrowLeft />
        ...
        <motion.div
          ...
          <div className="lg:w-8/12">
            <motion.div
              ...
              <CartTable cartProducts={cartProducts} total={finalTotal} />
            </motion.div>
              ...
              <div className="p-6">
                  ...
                  <div className="flex flex-col  gap-4">
                    <div className="flex gap-2 items-center">
                      <input
                        ...
                        className="flex-1 px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-400 dark:bg-gray-700 dark:text-white"
                      />
                      <Button
                        onClick={() => handleCoupon(couponCode)}
                        disabled={userTakeCoupon}
                        type="button"
                        className=" bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-medium transition-all cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed">
                        Apply
                      </Button>
                    </div>
                    {couponStatus && (
                      <div className="flex  gap-3 items-center ">
                        <p className="flex-1 text-red-600 mt-2 border border-red-600 rounded-lg p-2">
                          {couponStatus}
                        </p>
                        <Button
                          onClick={() => setCouponStatus("")}
                ...
                <div className="flex flex-wrap items-center justify-center gap-2 mt-8 pt-6 border-t border-gray-100 dark:border-gray-700">
                  <img
                    ...
                    alt="Visa"
                    className="h-8 grayscale opacity-60 hover:grayscale-0 hover:opacity-100 transition-all cursor-pointer"
                  />
                  <img
