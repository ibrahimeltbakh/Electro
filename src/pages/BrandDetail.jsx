...
import { FaFilter, FaThLarge, FaThList, FaCheck, FaSearch, FaArrowLeft, FaShoppingCart } from 'react-icons/fa';
...
const BrandDetail = () => {
  ...
  const { data: brandsData, isLoading: brandsLoading, isError: brandsError } = useBrands();
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 py-10">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div 
          ...
          className="mb-8"
        >
          <Link to="/brands" className="inline-flex items-center gap-2 text-blue-600 hover:text-blue-800 mb-6">
                  ...
                  <div className="flex items-center gap-6">
                    ...
                    <div>
                      ...
                      <p className="text-indigo-100 text-sm sm:text-base drop-shadow-md max-w-lg">
                        {currentBrand.description || `${currentBrand.name} is a leading manufacturer of high-quality products. Browse our collection of premium ${currentBrand.name} items.`}
                      </p>
                    </div>
                  </div>
                </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md overflow-hidden">
            <div className="grid md:grid-cols-2 gap-0">
              <div className="bg-gradient-to-br from-indigo-500 to-purple-600 p-8 flex flex-col justify-center">
                <h2 className="text-2xl font-bold text-white mb-4">About {currentBrand.name}</h2>
                <p className="text-indigo-100 mb-6">
                  {currentBrand.name} has been a trusted name in the industry for years, delivering exceptional products that combine innovation, quality, and style.
                </p>
                <ul className="space-y-3">
                  {['Premium Quality', 'Innovation', 'Customer Satisfaction', 'Warranty Coverage'].map((feature, index) => (
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className="hidden sm:flex items-center bg-gray-100 dark:bg-gray-700 rounded-lg overflow-hidden">
                <button 
                  ...
                  className={`p-2 transition-colors ${viewMode === 'grid' ? 'bg-indigo-600 text-white' : 'text-gray-500 dark:text-gray-400'}`}
                >
                  <FaThLarge />
                </button>
                <button 
                  onClick={() => setViewMode('list')} 
                  className={`p-2 transition-colors ${viewMode === 'list' ? 'bg-indigo-600 text-white' : 'text-gray-500 dark:text-gray-400'}`}
                >
                  <FaThList />
                </button>
              </div>
        ...
        {sortedProducts.length === 0 ? (
          ...
          <>
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-bold text-gray-800 dark:text-white">{currentBrand.name} Products</h2>
              <span className="text-sm text-gray-500 dark:text-gray-400">{sortedProducts.length} products found</span>
            </div>
              ...
              {sortedProducts.map((product, index) => (
                <motion.div 
                  ...
                  transition={{ 
                    duration: 0.5,
                    delay: index * 0.05,
                    type: viewMode === 'grid' ? ['spring', 'easeOut', 'easeIn', 'tween'][index % 4] : 'spring'
                  }}
        ...
        {sortedProducts.length > 0 && (
          <div className="mt-16 mb-8">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-bold text-gray-800 dark:text-white">Similar Products From Other Brands</h2>
              <Link to="/shop" className="text-indigo-600 hover:text-indigo-800 font-medium text-sm">View All Products</Link>
            </div>
                ...
                {productsData?.products
                  ?.filter(product => {
                    ...
                    return product.brand._id !== currentBrand._id && 
                          ...
                          filteredProducts.some(fp => fp.category._id === product.category._id);
                  })
                  .slice(0, 8)
                  .map(product => (
        ...
        {sortedProducts.length > 0 && (
          <div className="mt-16 mb-8">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-bold text-gray-800 dark:text-white">More From {currentBrand.name}</h2>
              <Link to="/shop" className="text-indigo-600 hover:text-indigo-800 font-medium text-sm">View All Products</Link>
            </div>
